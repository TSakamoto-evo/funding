...
void Discipline::assign_resources_equiv_unit(){
  ...
  for(int i = 0; i < paras.num_of_lottery_win; i++){
    tmp_resources.at(indices.at(i)) +=
      paras.total_resource * paras.p_lottery / paras.num_of_lottery_win;
  }

  //baseline
  for(int i = 0; i < paras.num_of_labs; i++){
    tmp_resources.at(i) += paras.total_resource *
      (1.0 - paras.p_competitive - paras.p_lottery) / paras.num_of_labs;
  }
