...
Discipline::Discipline( const Parameters para ):
  ...
  std::uniform_int_distribution<> det_w(0, paras.width - 1);
  std::uniform_int_distribution<> det_d(0, paras.depth - 1);

  std::vector<int> ini_x, ini_y;
  for(int i = 0; i < paras.ini_num; i++){
    ini_x.push_back(det_w(mt));
    ini_y.push_back(det_d(mt));
  }

  for(int i = 0; i < paras.num_of_labs; i++){
    labs.at(i).for_ini_pos(ini_x.at(i % paras.ini_num), ini_y.at(i % paras.ini_num));
  }
}
...
void Discipline::add_significance(const int num_peak, const double add_noise){
  for(int i = 0; i < num_peak; i++){
    landscape.add_peak(paras.sigma_peak.at(0), paras.height_peak.at(0));
  }

  landscape.add_noise(add_noise);
}
